...
- 約定・保有・損益の正本：SBI証券
- 判断・実行日時・マスター・保有事実の記録：非公開 `fdoa-app-data/progress.json`
- 銘柄検索：アプリに同梱した静的JSON（実行時APIなし）
- 参考株価：非公開 `fdoa-app-data/prices.json` を表示時だけ読み込む（`progress.json`・localStorageには保存しない）

保存の原則：**保有事実（数量・取得単価・買付日＝約定した過去の事実）は保存し、変動する時価・評価額・損益は保存しない**。ポートフォリオ全景は「保存した保有事実 × 最新quote（SBI一時反映があれば優先→無ければ参考株価）」を表示のたびに計算する常設パネルで、再読み込みしても消えない。SBI取込みは保有事実の更新手段（売買・受渡の後に1回実行すればよい）。
...
Safariの「SBI→PP」ブックマークをSBIポートフォリオ画面で実行すると、表示中の株式について比較判断に必要な保有情報をProgress Portfolioへ送る。

- 受信元はHTTPSのSBI証券ドメインだけを許可する
- 取得するのは銘柄コード・買付日・保有数・取得単価（米国株は参考単価）・現在値・前日比・損益金額・損益率・評価額
- SBIのログイン情報・口座番号・注文情報は取得しない
- 現在値・前日比・損益などの変動値はメモリー上だけでYahoo参考株価より優先し、GitHub・localStorageへ保存しない
- 保有数・取得単価（米国株は参考単価）・買付日は保有事実として`progress.json`へ保存する（ポートフォリオ全景の計算元）
- 再読み込みするとSBI由来の変動値は消えてYahoo参考株価へ戻る。保有事実は残る
...
2026-07-21から`stocks[].holding`＝保有事実（`quantity`・`costPrice`・`costLabel`・`acquisitionDate`・`updatedAt`）を保存する。SBI取込み成功時に取込み内容で上書き更新される。時価・評価額・損益のフィールドは存在しない（表示のたびに計算）。
