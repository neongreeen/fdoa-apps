...
function parseTile(sec){
  ...
  if((m=b.match(/(?:90日|126日|半年)高値\s*\*{0,2}(\$?[\d,.]+)(?:\s*円)?\*{0,2}\s*（(\d{4})-(\d{2})-(\d{2})）/)))
    ...
    t.peak={date:`${+m[1]}/${+m[2]}`};
  // 18:30監視時点の基準値（指数＝素の数値／個別株＝円建て・ドル建て。どちらも終値の日付を添える）
  // 2026-07-28：円建ても日付必須へ統一（旧「円建ては当日終値だから日付不要」を撤回。
  // 土日祝の手動実走で当日以外の終値が入り、日付なしだと最終チェック日で代用して1日ズレていた）
  // 拾う書式：「現在7,294円・7/24終値」「現在：**$113.50**（2026-07-27＝NY月曜終値）」
  const closeNote=(rest,mkt)=>{
    const dm=String(rest||"").split("／")[0].match(/(\d{4}-\d{2}-\d{2}|\d{1,2}\/\d{1,2})[^）\n]*終値/);
    if(!dm)return "";
    let d=dm[1];const iso=d.match(/^\d{4}-(\d{2})-(\d{2})$/);if(iso)d=(+iso[1])+"/"+(+iso[2]);
    return "（"+d+" "+mkt+"）";
  };
  ...
  else if((m=b.match(/現在[：:]?\s*\*{0,2}([\d,.]+)\s*円\*{0,2}([^\n]*)/)))t.price=m[1]+"円"+closeNote(m[2],"終値");
  else if((m=b.match(/現在[：:]?\s*\*{0,2}\$\s*([\d,.]+)([^\n]*)/)))t.price="$"+m[1]+closeNote(m[2],"NY終値");
  // 下落理由メモ（4色＝重さ分類）
  if((m=b.match(/理由[：:]\s*(🔴|🟡|🟢|🔵)\s*(.+)/)))
